...
impl PolicyEnforcer {
    ...
    pub fn check_tool_action(&self, tool: &str, action: &str) -> crate::Result<()> {
        if let Some(ref caps) = self.toolcaps {
            if !caps.is_action_allowed(tool, action) {
                return Err(crate::RuntimeError::PolicyViolation(
                    format!("Tool action not allowed: {}::{}", tool, action)
                ));
            }
        }
        Ok(())
    ...
    pub fn check_network_domain(&self, domain: &str, policy: &wadah_spec::Policy) -> crate::Result<()> {
        if let Some(ref network_policy) = policy.network {
            // Check deny list first
            if network_policy.deny_domains.iter().any(|d| domain.contains(d)) {
                return Err(crate::RuntimeError::PolicyViolation(
                    format!("Domain denied: {}", domain)
                ));
            }

            // Check allow list
            if !network_policy.allow_domains.is_empty() 
                && !network_policy.allow_domains.iter().any(|d| domain.contains(d)) {
                return Err(crate::RuntimeError::PolicyViolation(
                    format!("Domain not in allow list: {}", domain)
                ));
            }
        }
        Ok(())
    ...
    pub fn check_filesystem_path(
        ...
        for deny_path in &policy.filesystem.deny_paths {
            if path.starts_with(deny_path) {
                return Err(crate::RuntimeError::PolicyViolation(
                    format!("Path denied: {}", path.display())
                ));
            }
        }
        ...
        if !policy.filesystem.allow_paths.is_empty() {
            ...
            if !allowed {
                return Err(crate::RuntimeError::PolicyViolation(
                    format!("Path not in allow list: {}", path.display())
                ));
            }
        }
        ...
        if is_write && policy.filesystem.read_only {
            return Err(crate::RuntimeError::PolicyViolation(
                format!("Write not allowed in read-only mode: {}", path.display())
            ));
        }
...
mod tests {
    ...
    fn test_filesystem_policy() {
        ...
        assert!(enforcer.check_filesystem_path(
