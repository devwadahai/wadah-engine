...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| format!("wadah={},wadah_runtime={}", log_level, log_level).into()),
        )
    ...
    // Print banner
    println!("{}", "╔════════════════════════════════════════╗".bright_cyan());
    println!("{}", "║    Wadah - Contain Intelligence        ║".bright_cyan());
    println!("{}", "║       A Vessel for Thought             ║".bright_cyan());
    println!("{}", "╚════════════════════════════════════════╝".bright_cyan());
    println!();
    
    // Execute command
    match cli.command {
        Commands::Init { name, output, security } => {
        ...
        Commands::Push { reference, package, insecure } => {
            commands::push::execute(&reference, &package, insecure).await?;
        }
        Commands::Pull { reference, output, insecure } => {
